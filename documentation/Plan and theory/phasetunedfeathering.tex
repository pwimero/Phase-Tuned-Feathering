...
\subsection{Ablation-oriented optimization plan}

The campaign runs the full-geometry optimizer under each of the three ablation
conditions.  Comparing results isolates the contribution of each mechanism.

\begin{longtable}{p{0.20\linewidth} p{0.34\linewidth} p{0.34\linewidth}}
\toprule
\textbf{Condition} & \textbf{What changes} & \textbf{Question answered} \\
\midrule
\endhead

Incidence only
&
Only $\alpha_i$ vary; all geometry locked to baseline.
&
Is incidence phase tuning alone sufficient, or is full geometry mutation needed?
\\

Zero coherence
&
Full geometry search, but $\Gamma_{mn}=\delta_{mn}$ (sources uncorrelated).
&
Does the array phase-tuning mechanism matter, or is directivity shaping purely an amplitude effect?
\\

No incidence delay
&
Full geometry search, but $b_\alpha=0$ (no incidence-dependent phase delay).
&
Does the incidence-delay closure contribute, or is spatial phase alone sufficient?
\\

Full model
&
All geometry variables and all closure features active.
&
Baseline for comparison; best achievable target fit under the complete model.
\\
...
\begin{itemize}
    ...
    \item ablation studies: zero-coherence (tests the core array mechanism), no-incidence-delay (tests the delay closure contribution), and incidence-only (tests whether geometry beyond incidence is needed);
